...
public class RyanMovement : MonoBehaviour
{
	...
	public float jumpForce = 50f;			//Initial force of jump
	public float crouchJumpBoost = 5f;	//Jump boost when crouching
	public float hangingJumpForce = 300f;	//Force of wall hanging jumo
	public float jumpHoldForce = 3.8f;		//Incremental force when jump is held
	...
	public float eyeHeight = 3.5f;			//Height of wall checks
	...
	Vector3 bodyColliderExtents;			//The extents of collider component
	...
	void Start ()
	{
		...
		//Get half of collider's size (both x & y)
		bodyColliderExtents = bodyCollider.bounds.extents;
	}
	...
	void PhysicsCheck()
	{
		...
		RaycastHit2D leftCheck = Raycast(new Vector2(-bodyColliderExtents.x, bodyColliderExtents.y / 2), Vector2.down, groundDistance);
		RaycastHit2D rightCheck = Raycast(new Vector2(bodyColliderExtents.x, bodyColliderExtents.y / 2), Vector2.down, groundDistance);
